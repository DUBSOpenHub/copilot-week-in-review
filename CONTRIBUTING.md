...
- `web/` — Next.js + Tailwind v4 + shadcn/ui source for the landing page.
- `docs/` — built static site committed to the repo. GitHub Pages serves it from `main` branch / `/docs` folder at [dubsopenhub.github.io/copilot-week-in-review](https://dubsopenhub.github.io/copilot-week-in-review/). Regenerated by `cd web && npm run deploy`.
...
cd web
npm install
npm run dev          # http://localhost:3000/copilot-week-in-review/
```

To redeploy the landing page after changes:

```bash
cd web
npm run deploy       # rebuilds, copies web/out → ../docs, adds .nojekyll
cd ..
